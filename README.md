...
## Образы: выбор и обоснование

### Backend

| Стадия | Образ | Размер | Обоснование |
|--------|-------|--------|-------------|
| Builder | `golang:1.26-alpine` | ~300 MB (не входит в финальный образ) | Официальный образ Go на Alpine вместо Debian-based (`golang:1.26`, ~900 MB) — ускоряет pull в CI. Используется только для компиляции. |
| Runtime | `alpine:3.21` | ~8 MB base | Минимальный дистрибутив без лишних пакетов. Go собирается с `CGO_ENABLED=0` в статический бинарь, поэтому glibc не нужен. Выбран вместо `scratch` — Alpine добавляет `wget` (healthcheck) и `ca-certificates` (HTTPS). |
| **Итого** | | **~25 MB** | |
...
| Стадия | Образ | Размер | Обоснование |
|--------|-------|--------|-------------|
| Builder | `node:18-alpine` | ~170 MB (не входит в финальный образ) | LTS-версия Node.js на Alpine, совместимая с Vue CLI 4 и webpack 4. Используется только для `npm run build`. |
| Runtime | `nginxinc/nginx-unprivileged:stable-alpine` | ~25 MB base | Официальный образ nginx от NGINX Inc. Ключевое отличие от `nginx:alpine` — запускается от непривилегированного пользователя на порту 8080, не требует `root`. `stable-alpine` — актуальная стабильная ветка на минимальном Alpine. |
| **Итого** | | **~35 MB** | |
